function InApp(){
  ...
  if(band==-1){ //se encontraron rerrores    
    crearDiagOK("EXPRESSTELESERVICES", msgError, false, 170, 430, "");
  } else{
	...
	switch(parseInt(rslt)){
	  case -1: //usuario no encontrado
	    crearDiagOK("EXPRESSTELESERVICES", 'Error: The user name does not correspond to any user of the system', true, 200, 450, "");
	    $('#txtUser').attr("value","");
            $('#txtClave').attr("value","");
            break;
	  case -2: //password incorrecto
            crearDiagOK("EXPRESSTELESERVICES", "Error: The password entered is wrong", true, 200, 450, "$('#txtClave').focus()");
            $('#txtClave').attr("value","");
	    break;
	  case -3: //usuario inactivo
            crearDiagOK("EXPRESSTELESERVICES", "Error: The user is Off! <br> Please contact the administrator", true, 200, 450, "$('#txtClave').focus()");
            break;
	  case 2:
            crearDiagOK("EXPRESSTELESERVICES", 'Welcome accessed the  intranet EXPRESSTELESERVICES', true, 200, 450, "location.href='home.php';");
            break;
	 }
      },
      error: function(rslt) {
        crearDiagOK("EXPRESSTELESERVICES", 'There was an error in the execution '+rslt+'. <br>try again', true, 200, 450, "");
      }
    });
  }	
}
...
function closeApp(){
  crearDiagYN("EXPRESSTELESERVICES", "Want Logout", true, 170, 370, "logOut();")
}
 ...
 crearDiagLoad("Procesando...", "cerrando sesi&oacute;n de usuario, espere...", true, 125,320);
  $.ajax({
    ...
    success: function(rslt){
      ...
      crearDiagOK("EXPRESSTELESERVICES", 'Ha habido un error en la ejecuci&oacuten '+rslt+'. <br>Intente nuevamente', true, 200, 450, "");
    }
  });
}
...
function usr_cancelPwd(){
  crearDiagYN("EXPRESSTELESERVICES","Desea salir sin guardar los datos?",true,170,350,"location.href='home.php';");
}
  ...
  if(usr_validaFrmPwd()){
    ...
    $.ajax({
      ...
      success: function(rslt){
        ...
        $("input").attr("disabled",false);
        if(parseInt(rslt)==2){
          crearDiagOK("EXPRESSTELESERVICES", 'Su contrase&ntilde;a ha sido cambiada satisfactoriamente ', true, 200, 400, "location.href='home.php'");
        } else{
          crearDiagOK("EXPRESSTELESERVICES", 'La contrase&ntilde;a actual es incorrecta, intente de nuevo', true, 200, 450, "");
          $('#pwdAct').focus();
        }
      }, error: function(rslt) {
        crearDiagOK("EXPRESSTELESERVICES", 'Ha habido un error en la ejecuci&oacuten '+rslt+'. <br>Intente nuevamente', true, 200, 450, "");
...
function usr_cancel(){
  crearDiagYN("EXPRESSTELESERVICES","Desea salir sin guardar los datos?",true,170,350,"loadPage('../usr/usr_list.php');");
}
  ...
  if(usr_validaForm()){
    ...
    $.ajax({
      ...
      success: function(rslt){
        $("#dialog-load").dialog("close");
        $("input").attr("disabled",false);
        crearDiagOK("ORTHOCENTER", 'Se ha creado el nuevo usuario satisfactoriamente ', true, 200, 400, "loadPage('../usr/usr_list.php')");
      }, error: function(rslt) {
        crearDiagOK("ORTHOCENTER", 'Ha habido un error en la ejecuci&oacuten '+rslt+'. <br>Intente nuevamente', true, 200, 450, "");
      }
    });  
  }
}
...
function usr_update(){  
  ...
  $.ajax({
    ...
    success: function(rslt){
      $("#dialog-load").dialog("close");
      $("input").attr("disabled",false);
      crearDiagOK("EXPRESSTELESERVICES", 'Los datos del usuario han sido actualizados! ', true, 200, 400, "loadPage('../usr/usr_list.php')");
    }, error: function(rslt) {
      crearDiagOK("EXPRESSTELESERVICES", 'Ha habido un error en la ejecuci&oacuten '+rslt+'. <br>Intente nuevamente', true, 200, 450, "");
    }
  });
...
function usr_del(idU){ //confirmación para borrar el registro seleccionado.
  crearDiagYN("EXPRESSTELESERVICES", "Est&aacute; segur@ de querer borrar este usuario", true, 170, 400, "usr_borrar("+idU+")");
}
