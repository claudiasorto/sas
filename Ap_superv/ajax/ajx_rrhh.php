...
switch($_POST['Do']){
	...
	case 'newAp': //Nueva acción de personal
		...
		$filtro = '';
	  	...
	  	$sqlText = "SELECT id_tpap , name_tpap ".
	  				"FROM type_ap WHERE CURRENT_DATE() <= IFNULL(END_DATE,CURRENT_DATE()) ".
	  				"AND name_tpap <> 'INGRESO DE EMPLEADO' ";
	...
	case 'emp'://Función para desplegar los empleados segun su estatus, su supervisor y su rol
		...
		if($_POST['idAp']==3){ //si la ap es traslados mostramos todos los empleados activos y a los cuales no supervisa
			if($_SESSION['usr_rol']=='SUPERVISOR'){
				...
				$filtro = " and id_role<".$_SESSION['usr_idrol'];
			}
	...
	case 'frmap': //Formulario de nueva ap según selección  
		...
		else if($_POST['ap']>=9 and $_POST['ap']<=14){
				$rslt = cargaPag("../mtto/ap_retiros.php");
		}
		/*if($_POST['ap']==16){
			$rslt = cargaPag("../mtto/ap_contratoAviso.php");
		}*/
		//Formulario de AP's standard a partir de la configuracion
		else{
			$rslt = cargaPag("../mtto/ap_generica.php");
		}
		
		
		$sqlText = "select e.EMPLOYEE_ID, ID_SUPERVISOR, USERNAME, FIRSTNAME, LASTNAME, dui,date_format(date_admis,'%d/%m/%Y') as d, ".
			"pe.id_plxemp as idplxemp, d.ID_DEPART, NAME_DEPART, cc.ID_ACCOUNT, NAME_ACCOUNT, pc.ID_PLACE, NAME_PLACE, ".
			"pd.id_placexdep, pj.job_type_id, pj.job_type_name ".
			...
			"inner join places pc on pc.id_place = pd.id_place ".
			"inner join account cc on pd.id_account=cc.id_account ".
			"where e.EMPLOYEE_ID=".$_POST['emp'].
			" and (pe.status_plxemp = 'A' or ".
				"(e.user_status = 'I' and pe.id_plxemp = ".
    			"(select max(pep.id_plxemp) from plazaxemp pep where pep.employee_id = e.employee_id)) )";
		...
		$optSuperv = '<select id="lsSuperv" class="txtPag">';
		foreach($dtSupervisor as $dtSup){
			$sel = "";
			if($_SESSION['usr_id']==$dtSup['employee_id']){
				$sel = "selected";	
			}
			$optSuperv .='<option value="'.$dtSup['employee_id'].'" '.$sel.'>'.$dtSup['firstname'].' '.$dtSup['lastname'].'</option>';
		}
		$optSuperv .='</select>';
		...
		$dtJt = $dbEx->selSql($sqlText);
	  	...
	  	foreach($dtJt as $dtJt){
	  	    $sel = "";
			if($dtE['0']['job_type_id']==$dtJt['JOB_TYPE_ID']){
				$sel = "selected";
			}
			$optJt .='<option value = "'.$dtJt['JOB_TYPE_ID'].'" '.$sel.'>'.$dtJt['JOB_TYPE_NAME'].'</option>';
		}
		...
		$rslt = str_replace("<!--optTipoPlaza-->",$optJt,$rslt);
				
		//Muestra los datos de la ap
		$sqlText2 = "select id_tpap, name_tpap, desc_tpap, has_start_date, has_end_date, has_time ".
					" from type_ap where ID_TPAP=".$_POST['ap'];
		...
		//Datos para la AP Generica
		if($dtAp['0']['has_start_date'] == 'Y'){
			$rslt = str_replace("<!--genStartDate-->", 
						'<tr><td align="right" class="txtPag">Fecha Inicial:</td><td><input type="text" name="fecha_inicio" id="fecha_inicio" size="15" class="txtPag" readonly="1" /><img src="images/calendar.jpg" onclick="return showCalendar('."'fecha_inicio'".', '."'%d/%m/%Y'".');" style="cursor:pointer;" /></td></tr>',
						$rslt);

		}
		if($dtAp['0']['has_end_date'] == 'Y'){
			$rslt = str_replace("<!--genEndDate-->", 
						'<tr><td align="right" class="txtPag">Fecha Final:</td><td><input type="text" name="fecha_fin" id="fecha_fin" size="15" class="txtPag" readonly="1" /><img src="images/calendar.jpg" onclick="return showCalendar('."'fecha_fin'".', '."'%d/%m/%Y'".');" style="cursor:pointer;" /></td></tr>',
						$rslt);

		}
		if($dtAp['0']['has_time'] == 'Y'){
			$rslt = str_replace("<!--genTime-->", 
						'<tr><td align="right" class="txtPag">Horas:</td>'.
						'<td><select id="txtHoras" name="txtHoras" class="txtPag">'.$optHoras.'</select>'.
						'</td></tr>',
						$rslt);
	...
	case 'sendNotification':
		...
		$rsltMail = $apFun->sendNotification($_POST['idAP']);
	...
	case 'sv_traslados': //guarda las ap de traslados
		...
		$sqlText = "update plazaxemp set status_plxemp='I', end_date = CURDATE() where id_PLXEMP=".$_POST['idPxe'];
		$dbEx->updSql($sqlText);
		
		//ingresa la nueva plaza del empleado
		$sqlText = 	"insert into plazaxemp set id_placexdep =".$_POST['plaza'].", employee_id=".$_POST['idE'].", status_plxemp='A', ".
			" start_date = CURDATE(), id_apxemp=".$rslt.", job_type_id = ".
			" (select pe.job_type_id from plazaxemp pe where pe.id_PLXEMP=".$_POST['idPxe'].") ";
	...
	case 'sv_vacaciones': //guarda las ap de vacaciones
		...
		//Si se inserto el nuevo registro enviar notificacion
		echo $rslt;
	break;
	...
	case 'sv_puesto': //Guarda una ap de nuevo puesto/periodo de prueba
		...
		$sqlText = "update employees set id_supervisor=".$_POST['supervisor'].", salary=".$_POST['salario']." where employee_id=".$_POST['idE'];
		$dbEx->updSql($sqlText);
		$insert = "";
		...
		$sqlText = "update plazaxemp set status_plxemp='I', end_date = CURDATE() where id_plxemp=".$dtPlazaOld['0']['id_plxemp'];
		$dbEx->updSql($sqlText);
		
		$sqlText = "insert into plazaxemp set id_placexdep=".$dtPlazaNew.", employee_id=".$_POST['idE'].", ".
			"pprueba_plxemp=".$_POST['prueba'].", status_plxemp='A', id_apxemp=".$idC.", start_date = CURDATE(), ".
			"job_type_id = ".$_POST['plaza']." ";
	...
	case 'saveup_puesto': //Actualiza ap por puesto
		...
		$dtE = $dbEx->selSql($sqlText);
		$sqlText = " update employees set salary=".$_POST['salario'].", id_supervisor=".$_POST['supervisor']." where employee_id=".$dtE['0']['employee_id'];
		$dbEx->updSql($sqlText);
		$sqlText = " update plazaxemp set pprueba_plxemp=".$_POST['prueba'].", id_placexdep=".$_POST['posicion'].", ".
		    " job_type_id = ".$_POST['plaza'].
	...
	case 'loadApxE': //Muestra la ap recien guardada
		$sqlText = "select ape.ID_APXEMP, ape.ID_TPAP, ape.EMPLOYEE_ID, ".
						"ape.ID_CENTER, ape.STARTDATE_AP, ape.ENDDATE_AP, ape.HOURS_AP, ".
						"ape.STORAGEDATE_AP, ape.ID_TPDISCIPLINARY, ape.TYPESANCTION_AP, ".
						"ape.TYPEINCAP_AP, ape.APPROVED_EMP, ".
    					"ape.AUTOR_AP, IFNULL(ape.AUTOR_WORK,0) AUTOR_WORK, ape.APPROVED_WORK, ".
    					"IFNULL(ape.AUTOR_AREA,0) AUTOR_AREA, ape.APPROVED_AREA, ".
    					"IFNULL(ape.AUTOR_HR,0) AUTOR_HR, ape.APPROVED_HR, ".
    					"IFNULL(ape.AUTOR_GENERALMAN,0) AUTOR_GENERALMAN, ape.APPROVED_GENERAL, ".
    					"ape.COMMENT_AP, ape.STATUS_AP, ape.REJECTED_COMMENTS, ".
    					"ape.APPROVED_STATUS, ape.ID_PLACEXDEP_NEW, ape.ID_PLACEXDEP_OLD, ".
    					"ape.SUPERVISOR_NEW, ape.SUPERVISOR_OLD, ".
						"date_format(startdate_ap,'%d/%m/%Y') as start, ".
						"date_format(enddate_ap,'%d/%m/%Y') as end, ".
						"date_format(storagedate_ap,'%d/%m/%Y') as stg, ".
						"date_format(date_admis,'%d/%m/%Y') as admis, ".
						" replace(replace(replace(comment_ap,'#','No'),'\n',''),char(34 using ASCII),'' ) comment_ap_replace, ".
						" em.*, ta.* ".
					" from apxemp ape inner join employees em on ape.employee_id = em.employee_id ".
					" inner join type_ap ta on ta.id_tpap = ape.id_tpap ".
					" where id_apxemp=".$_POST['id'];
		$dtAp = $dbEx->selSql($sqlText);
		
		//verifica las firmas

		$sqlText = "select pd.id_role, name_role from employees e inner join plazaxemp pe on e.employee_id=pe.employee_id inner join placexdep pd on pd.id_placexdep=pe.id_placexdep inner join user_roles ur on ur.id_role=pd.id_role where pe.status_plxemp='A' and e.employee_id=".$dtAp['0']['AUTOR_AP'];
		$rol_autor = $dbEx->selSql($sqlText);
		
		$firmas = $apFun-> getFirmas($dtAp['0']['ID_APXEMP']);
		
		if($_POST['accion']==1){ //Si la accion es ver las ap
			if($dtAp['0']['ID_TPAP']==1 or $dtAp['0']['ID_TPAP']==2){
				$rslt = cargaPag("../mtto/load_permiso.php");	
			}
			else if($dtAp['0']['ID_TPAP']==3){
				$rslt = cargaPag("../mtto/load_traslado.php");	
			}
			else if($dtAp['0']['ID_TPAP']==4){
				$rslt = cargaPag("../mtto/load_reingreso.php");	
			}
			else if($dtAp['0']['ID_TPAP']==5){
				$rslt = cargaPag("../mtto/load_vacaciones.php");
				}
			else if($dtAp['0']['ID_TPAP']==6){
				$rslt = cargaPag("../mtto/load_disciplinarias.php");
			}
			else if($dtAp['0']['ID_TPAP']==7){
				$rslt = cargaPag("../mtto/load_incapacidad.php");	
			}
			else if($dtAp['0']['ID_TPAP']==8 or $dtAp['0']['ID_TPAP']==15){
				$rslt = cargaPag("../mtto/load_puesto.php");	
			}
			else if($dtAp['0']['ID_TPAP']>=9 and $dtAp['0']['ID_TPAP']<=14){
				$rslt = cargaPag("../mtto/load_retiros.php");	
			}
			else{
				$rslt = cargaPag("../mtto/load_apgenerica.php");	
			}
			...
			if($dtAp['0']['APPROVED_AREA']!='N' and $dtAp['0']['APPROVED_WORK']!='N' and $dtAp['0']['APPROVED_HR']!='N' and $dtAp['0']['APPROVED_GENERAL']!='N'){ //Si la AP fue rechazada permite al sup actualizar
				...
				if($dtAp['0']['ID_TPAP']==1 or $dtAp['0']['ID_TPAP']==2){
					...
					$rslt = cargaPag("../mtto/up_generica.php");		
				}
			}
		}
		if($_POST['autorizar']==1){
			...
			$back = '<input type="image" src="images/back2.png" align="absmiddle" width="40" alt="Regresar" title="Regresar a lista de acciones pendientes de aprobaci&oacute;n" style="cursor:pointer;" onClick="autorizarap()">';
			$rslt = str_replace("<!--btn_autor-->",$autor,$rslt);
			$rslt = str_replace("<!--btn_rechaz-->",$rechaz,$rslt);	
			$rslt = str_replace("<!--btn_back-->", $back, $rslt);
		}
		else{
		...
		$rslt = str_replace("<!--firmas-->",$firmas,$rslt);
		...
		//Texto variable para ap generica
		if($dtAp['0']['has_start_date'] == 'Y'){
			$rslt = str_replace("<!--genStartDate-->", 
						'<tr><td align="right" class="txtPag">Fecha Inicial:</td><td class="txtPag">'.$dtAp['0']['start'].'</td></tr>',
						$rslt);

			$rslt = str_replace("<!--genUpStartDate-->", 
						'<tr><td align="right" class="txtPag">Fecha Inicial:</td><td><input type="text" name="fecha_ini" id="fecha_ini" size="15" class="txtPag" readonly="1" value="'.$dtAp['0']['start'].'" /><img src="images/calendar.jpg" onclick="return showCalendar('."'fecha_ini'".', '."'%d/%m/%Y'".');" style="cursor:pointer;" /></td></tr>',
						$rslt);

		}
		if($dtAp['0']['has_end_date'] == 'Y'){
			$rslt = str_replace("<!--genEndDate-->", 
						'<tr><td align="right" class="txtPag">Fecha Final:</td><td class="txtPag">'.$dtAp['0']['end'].'</td></tr>',
						$rslt);

			$rslt = str_replace("<!--genUpEndDate-->", 
						'<tr><td align="right" class="txtPag">Fecha Final:</td><td><input type="text" name="fecha_fin" id="fecha_fin" size="15" class="txtPag" readonly="1" value="'.$dtAp['0']['end'].'" /><img src="images/calendar.jpg" onclick="return showCalendar('."'fecha_fin'".', '."'%d/%m/%Y'".');" style="cursor:pointer;" /></td></tr>',
						$rslt);

		}
		if($dtAp['0']['has_time'] == 'Y'){
			$rslt = str_replace("<!--genTime-->",
						'<tr><td align="right" class="txtPag">Horas:</td><td class="txtPag">'.$dtAp['0']['HOURS_AP'].'</td></tr>',
						$rslt);

			$rslt = str_replace("<!--genUpTime-->", 
						'<tr><td align="right" class="txtPag">Horas:</td>'.
						'<td><select id="txtHoras" name="txtHoras" class="txtPag">'.$optHoras.'</select>'.
						'</td></tr>',
						$rslt);
		...
		$sqlText = "select pd.ID_PLACEXDEP, pd.ID_DEPART, NAME_DEPART, pd.ID_ACCOUNT, NAME_ACCOUNT, pd.ID_PLACE, NAME_PLACE, ".
			"date_format(date_admis,'%d/%m/%Y') as d, pe.id_plxemp as idplxemp, e.id_supervisor, ".
			"jt.job_type_id, jt.job_type_name ".
			"from employees e inner join plazaxemp pe on e.employee_id=pe.employee_id ".
			"inner join job_type jt on jt.job_type_id = pe.job_type_id ".
			"inner join placexdep pd on pe.id_placexdep = pd.id_placexdep ".
			"inner join depart_exc d on pd.ID_DEPART = d.ID_DEPART ".
			"inner join places pc on pc.id_place = pd.id_place ".
			"inner join account cc on pd.id_account=cc.id_account ".
			"where e.EMPLOYEE_ID=".$dtAp['0']['EMPLOYEE_ID']." and status_plxemp='A'".
			"union all ".
			"select null ID_PLACEXDEP, null ID_DEPART, null NAME_DEPART, null ID_ACCOUNT, null NAME_ACCOUNT, null ID_PLACE, ".
			"null NAME_PLACE, date_format(date_admis,'%d/%m/%Y') as d, null id_plxemp, e.id_supervisor, ".
			"null job_type_id, null job_type_name ".
            "from employees e inner join employee_status es on es.status_id = e.user_status ".
            "where es.status_name = 'Inactivo' ".
            "and e.EMPLOYEE_ID=".$dtAp['0']['EMPLOYEE_ID'];
		...
		$rslt = str_replace("<!--plaza-->", $dtE['0']['NAME_PLACE'],$rslt);
		$rslt = str_replace("<!--idplxemp-->",$dtE['0']['idplxemp'],$rslt);
		$rslt = str_replace("<!--tipo_plaza_id-->",$dtE['0']['job_type_id'],$rslt);
		$rslt = str_replace("<!--tipo_plaza-->",$dtE['0']['job_type_name'],$rslt);
		...
		$dtJt = $dbEx->selSql($sqlText);
	  	...
	  	foreach($dtJt as $dtJt){
			...
			$optPl .='<option value = "'.$dtJt['JOB_TYPE_ID'].'" '.$sel.'>'.$dtJt['JOB_TYPE_NAME'].'</option>';
		}
        $rslt = str_replace("<!--optTipoPlaza-->",$optPl,$rslt);
		
		if($dtAp['0']['ID_PLACEXDEP_NEW']>0){
			$sqlText = "select p.id_place, name_place, c.id_account, name_account, d.id_depart, name_depart ".
				"from placexdep pd inner join places p on p.id_place=pd.id_place ".
				"inner join account c on c.id_account=pd.id_account ".
				"inner join depart_exc d on d.id_depart=pd.id_depart ".
				"where id_placexdep=".$dtAp['0']['ID_PLACEXDEP_NEW'];
			...
			$sqlText = "select name_place, name_account, name_depart from placexdep pd ".
				"inner join places p on p.id_place=pd.id_place ".
				"inner join account c on c.id_account=pd.id_account ".
				"inner join depart_exc d on d.id_depart=pd.id_depart ".
				"where id_placexdep= ".$dtAp['0']['ID_PLACEXDEP_OLD'];
		...
		if($dtAp['0']['ID_TPAP']==3){
			...
			$sqlText = "select d.id_depart, d.name_depart, c.id_account, c.name_account, ".
				"po.id_place, po.name_place ".
				"from placexdep pd inner join depart_exc d on pd.id_depart=d.id_depart ".
 				"inner join places po on po.id_place=pd.id_place ".
 				"inner join account c on c.id_account=pd.id_account ".
 				"where pd.id_placexdep = ".$dtAp['0']['ID_PLACEXDEP_OLD'];
		...
		if($_SESSION['usr_rol']!='RECURSOS HUMANOS'){
			...
			$sqlText = "select ac.name_account as na, ac.id_account as ida from account ac inner join placexdep pd on ac.id_account = pd.id_account ".
				"inner join plazaxemp pe on pe.id_placexdep=pd.id_placexdep ".
				"where pe.status_plxemp='A' and pe.employee_id=".$_SESSION['usr_id'];
			$dtC = $dbEx->selSql($sqlText);
		...
		//Seleccion disponible para empleados activos
		if(strlen($dtE['0']['ID_ACCOUNT'])>0){
			//Lista Departamentos de la cuenta
			$sqlText = "select distinct (d.id_depart) as idD, name_depart ".
				"from depart_exc d inner join placexdep pd on d.id_depart=pd.id_depart ".
				"inner join account c on pd.id_account=c.id_account where c.id_account=".$dtE['0']['ID_ACCOUNT'];
			$dtD = $dbEx->selSql($sqlText);
			...
			$sqlText = "select id_placexdep, name_place from places pl inner join placexdep pd on pd.id_place=pl.id_place ".
				"where pd.id_depart= ".$dtE['0']['ID_DEPART']." and pd.id_account=".$dtE['0']['ID_ACCOUNT'];
			$dtP = $dbEx->selSql($sqlText);
			$optPl = '';
			foreach($dtP as $dtp){
			...
			$sqlText = "select e.employee_id, firstname, lastname from employees e inner join plazaxemp pe on pe.employee_id= e.employee_id ".
				"inner join placexdep pd on pd.id_placexdep=pe.id_placexdep ".
				"inner join places pl on pl.id_place=pd.id_place ".
				"where pe.status_plxemp='A' and pl.nivel_place=2 and ".
				"pd.id_depart=".$dtE['0']['ID_DEPART']." and pd.id_account=".$dtE['0']['ID_ACCOUNT'];
	...
	case 'reportap':  //Genera las opciones de filtrado para nuevo reporte
		...
		//$filtro_ap = " where id_tpap in (1,2,3,4,5,6,7,9,13,16) ";
		$filtro_ap = " where CURRENT_DATE() <= IFNULL(END_DATE,CURRENT_DATE()) ";
		if($_SESSION['usr_rol']=='SUPERVISOR'){
			$sqlText = "select name_place from places pl inner join placexdep pd on pl.id_place=pd.id_place ".
						"inner join plazaxemp pe on pe.id_placexdep=pd.id_placexdep where pe.employee_id=".$_SESSION['usr_id']." ";
			$filtro1 = ' where ';
			$id_pl = $dbEx->selSql($sqlText);

			$filtro1 .= " e.id_supervisor=".$_SESSION['usr_id']." or e.employee_id=".$_SESSION['usr_id'];

			$sqlText = "select ac.name_account, ac.id_account from account ac ".
				"where ac.id_account in ".
				"(select c.id_account from account c inner join placexdep pd on c.id_account = pd.id_account ".
				"inner join plazaxemp pe on pe.id_placexdep=pd.id_placexdep ".
				"inner join employees e on e.employee_id=pe.employee_id ".$filtro1.
				" and pe.status_plxemp='A' and user_status=1)";
			...
			$sqlText = "select distinct(d.id_depart) as idD, name_depart from depart_exc d inner join placexdep pd on d.id_depart=pd.id_depart ".
				"inner join user_roles ur on ur.id_role=pd.id_role ".
				"inner join plazaxemp pe on pe.id_placexdep=pd.id_placexdep ".
				"inner join employees e on e.employee_id = pe.employee_id ".$filtro2." and pe.status_plxemp='A' and user_status=1";
	...
	case 'loadrpt': //Carga reporte segun filtros seleccionados
		...
		$filtroInact .= " where es.status_name = 'Inactivo' ";
		$estado = "";
		if($_SESSION['usr_rol']=='SUPERVISOR'){
			$filtro .= " and e.id_supervisor=".$_SESSION['usr_id'];
			$filtroInact .= " and e.id_supervisor=".$_SESSION['usr_id'];
		}
		...
		if($_POST['idAp']>0){
			...
			$filtroInact .= " and ap.id_tpap =".$_POST['idAp'];
			}
		if($_POST['idAg']>0){
			$filtro .= " and e.employee_id =".$_POST['idAg'];
            $filtroInact .= " and e.employee_id =".$_POST['idAg'];
			}
		...
		if(isset($_POST['emp']) && $_POST['emp']!=''){
			...
			$filtroInact .= " and (e.firstname like '%".strtoupper($_POST['emp'])."%' or e.lastname like '%".strtoupper($_POST['emp'])."%')";
		}
		if(isset($_POST['badge']) && $_POST['badge']!=''){
			 $filtro .= " and (e.username like '%".strtoupper($_POST['badge'])."%')";
			 $filtroInact .= " and (e.username like '%".strtoupper($_POST['badge'])."%')";
		}
		if($_POST['estado']>0){
			if($_POST['estado']==1){
				$estado = 1;
		...
		$sqlText = "select ap.id_apxemp, e.employee_id, e.username, e.firstname, e.lastname, tp.id_tpap, tp.name_tpap, ".
			...
			"date_format(ap.storagedate_ap,'%d/%m/%Y') as f2, date_format(ap.enddate_ap,'%d/%m/%Y') as f3, ".
			...
			"inner join placexdep pd on pd.id_placexdep=pe.id_placexdep ".$filtro.
		" union all ".
		"select ap.id_apxemp, e.employee_id, e.username, e.firstname, e.lastname, tp.id_tpap, tp.name_tpap, ".
			...
			"from employees e inner join apxemp ap on ap.employee_id=e.employee_id ".
			"inner join type_ap tp on tp.id_tpap=ap.id_tpap ".
            "inner join employee_status es on es.status_id = e.user_status ".$filtroInact.
			" order by 1 desc";

		$dtAp = $dbEx->selSql($sqlText);
		$rslt .= '<table cellpadding="3" cellspacing="0" width="950" class="tblResult" align="center" bordercolor="#069">';
		if($dbEx->numrows>0){
			$rslt .= '<tr class="txtPag"><td align="right" colspan="9"><form target="_blank" action="report/xls_rptaptotal.php" method="post"><input type="image" src="images/excel.png" alt="Exportar a excel" width="30" style="cursor:pointer" title="Exportar a excel" />&nbsp;&nbsp;<input type="hidden" name="filtro" value="'.$filtro.'"><input type="hidden" name="filtroinact" value="'.$filtroInact.'"></td><td></td></tr>';
			...
			foreach($dtAp as $dtAp){
				...
				if($estado == 1){
					$pasa = $apFun-> verificarAprobAp($dtAp['id_apxemp']);
				}
				else if($estado == 2){ //Ap pendientes
					$rechazadas = 0;
					$aprobadas = $apFun-> verificarAprobAp($dtAp['id_apxemp']);
	...
	case 'autorizarap': //Muestra ap por autorizar segun el rol del usuario
		...
		$sqlText = $apFun->getApPendientes($_SESSION['usr_rol']);
		$dtAp = $dbEx->selSql($sqlText);		
		...
		if($dbEx->numrows>0){
			$rslt .= '<tr><td colspan="10" align="center"><b>ACCIONES DE PERSONAL PENDIENTES DE AUTORIZAR<b></td></tr>';
			$rslt .= '<tr><td colspan="10">Total de Acciones de Personal: '.$dbEx->numrows.'</td></tr>';
			$rslt .= '<tr class="showItem"><td width="3%">AP#</td>'.
				'<td width="5%">BADGE</td>'.
				'<td width="20%">EMPLEADO</td>'.
				'<td width="10">CUENTA</td>'.
				'<td width="15%">SUPERVISOR</td>'.
				'<td width="5%">FECHA DE ELABORACION</td>'.
				'<td width="5%">FECHA EFECTIVA</td>'.
				'<td width="10%">TIPO DE AP</td>'.
				'<td width="4%">NUMERO HORAS</td>'.
				'<td width="20%">COMENTARIOS</td>'.
				'<td width="4%"></td></tr>';
			foreach($dtAp as $dtAp){

					$rslt .= '<tr class="rowCons">'.
						'<td onclick="loadAutor('.$dtAp['id_apxemp'].')">'.$dtAp['id_apxemp'].'</td>'.
						'<td onclick="loadAutor('.$dtAp['id_apxemp'].')">'.$dtAp['username'].'</td>'.
						'<td onclick="loadAutor('.$dtAp['id_apxemp'].')">'.$dtAp['firstname'].'&nbsp;'.$dtAp['lastname'].'</td>'.
						'<td onclick="loadAutor('.$dtAp['id_apxemp'].')">'.$dtAp['name_account'].'</td>'.
						'<td onclick="loadAutor('.$dtAp['id_apxemp'].')">'.$dtAp['supervisor'].'</td>'.
						'<td onclick="loadAutor('.$dtAp['id_apxemp'].')">'.$dtAp['f2'].'</td>'.
						'<td onclick="loadAutor('.$dtAp['id_apxemp'].')">'.$dtAp['f1'].'</td>'.
						'<td onclick="loadAutor('.$dtAp['id_apxemp'].')">'.$dtAp['name_tpap'].'</td>'.
						'<td onclick="loadAutor('.$dtAp['id_apxemp'].')">'.$dtAp['hours_ap'].'</td>'.
						'<td onclick="loadAutor('.$dtAp['id_apxemp'].')">'.$dtAp['comment_ap'].'</td>'.
						'<td><img src="images/elim.png" title="Click para eliminar" onclick="deleteAp('.$dtAp['id_apxemp'].')"></td>'.
						'</tr>';
			}
		}
		else{
	...
	case 'ResultAutorAp':
		...
		$result = $apFun-> verificarAprobAp($_POST['id']);
	...
	case 'ResultRechazAp':
		...
		if($_SESSION['usr_rol']=='GERENTE DE AREA'){
			$update .= "autor_area=".$_SESSION['usr_id'].", approved_area = 'N', approved_status = 'R' ";
		}
		else if($_SESSION['usr_rol']=='WORKFORCE'){
			$update .= "autor_work=".$_SESSION['usr_id'].", approved_work = 'N', approved_status = 'R' ";
		}
		else if($_SESSION['usr_rol']=='RECURSOS HUMANOS'){
			$update .= "autor_hr=".$_SESSION['usr_id'].", approved_hr = 'N', approved_status = 'R' ";
		}
		else if($_SESSION['usr_rol']=='GERENCIA'){
			$update .= "autor_generalman=".$_SESSION['usr_id'].", approved_general = 'N', approved_status = 'R' ";
		}
	...
	case 'loadApRechazada':
		...
		$sqlText = $apFun-> getDatosAp($_POST['id']);
		$dtE = $dbEx->selSql($sqlText);

		$sqlText = $apFun-> getPersonasAp($_POST['id']);
		$dtP = $dbEx->selSql($sqlText);
		...
		$back = '<input type="image" src="images/back2.png" align="absmiddle" width="40" alt="Regresar" title="Regresar a la lista" style="cursor:pointer;" onClick="'.$_POST['Origen'].'">';
		

		$print = '<a href="mtto/impRejectAp.php?idAp='.$dtE['0']['id_apxemp'].'&badge='.$dtE['0']['username'].'&nombre='.$dtE['0']['firstname'].'&apellido='.$dtE['0']['lastname'].'&cta='.$dtE['0']['name_account'].'&dpto='.$dtE['0']['name_depart'].'&posicion='.$dtE['0']['name_place'].'&nametpap='.$dtE['0']['name_tpap'].'&stg='.$dtE['0']['f3'].'&ini='.$dtE['0']['f1'].'&autor='.$dtP['0']['creado_por'].'&autorRec='.$dtP['0']['rechazado_por'].'&cargo='.$dtP['0']['plaza_rechazado'].'&comment='.$dtE['0']['rejected_comments'].'&commentAccion='.$comentario.'" target="_blanck"><img src="images/Print.png" border="0" width="50" style="cursor:pointer;" title="Pulse para imprimir" align="absmiddle" /></a>';
		...
		$rslt = str_replace("<!--autor_ap-->",$dtP['0']['creado_por'],$rslt);
		$rslt = str_replace("<!--autor_negar-->",$dtP['0']['rechazado_por'],$rslt);
		$rslt = str_replace("<!--cargo_negar-->",$dtP['0']['plaza_rechazado'],$rslt);
	...
	case 'reporteApRechazada':
		...
		if($dbEx->numrows>0){
			...
			foreach($dtAp as $dtA){
				$rslt .='<tr class="rowCons">'.
				'<td onclick="loadApRechazada('.$dtA['id_apxemp'].','.'reporteApRechazada'.')">'.$dtA['id_apxemp'].'</td>'.
				'<td onclick="loadApRechazada('.$dtA['id_apxemp'].','.'reporteApRechazada'.')">'.$dtA['name_tpap'].'</td>'.
				'<td onclick="loadApRechazada('.$dtA['id_apxemp'].','.'reporteApRechazada'.')">'.$dtA['username'].'</td>'.
				'<td onclick="loadApRechazada('.$dtA['id_apxemp'].','.'reporteApRechazada'.')">'.$dtA['firstname'].' '.$dtA['lastname'].'</td>'.
				'<td onclick="loadApRechazada('.$dtA['id_apxemp'].','.'reporteApRechazada'.')">'.$dtA['f3'].'</td></tr>';
			}
		}
		else{
	...
	case 'EditSuperv':
		...
		$rslt = str_replace("<!--optPosicion-->",$optP,$rslt);
			...
			$sqlText = "select distinct(e.employee_id), firstname, lastname from employees e inner join plazaxemp pe on pe.employee_id= e.employee_id inner join placexdep pd on pd.id_placexdep=pe.id_placexdep inner join places pl on pl.id_place=pd.id_place where pl.nivel_place=2 and pd.id_role=3 and user_status=1 and pe.status_plxemp='A' ";
			$dtS = $dbEx->selSql($sqlText);
			$optSup = '<select id="lsSuperv" class="txtPag")>';
	...
	case 'sv_apgenerica': //guarda las ap de permisos con y sin goce de sueldo
		$fec_ini = $oFec->cvDtoY($_POST['fec_ini']);
		$fec_fin = $oFec->cvDtoY($_POST['fec_fin']);
		$insert = "";
		if($_SESSION['usr_rol']=='GERENTE DE AREA'){
