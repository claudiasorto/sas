...
switch($_POST['Do']){
	...
	case 'sv_traslados': //guarda las ap de traslados
		...
		$sqlText = 	"insert into plazaxemp set id_placexdep =".$_POST['plaza'].", employee_id=".$_POST['idE'].", status_plxemp='A', ".
			" start_date = CURDATE(), id_apxemp=".$idC['0']['id'].", job_type_id = ".
	...
	case 'sv_puesto': //Guarda una ap de nuevo puesto/periodo de prueba
		...
		$sqlText = "insert into plazaxemp set id_placexdep=".$dtPlazaNew.", employee_id=".$_POST['idE'].", ".
			"pprueba_plxemp=".$_POST['prueba'].", status_plxemp='A', id_apxemp=".$idC['0']['id'].", start_date = CURDATE(), ".
	...
	case 'loadApxE': //Muestra la ap recien guardada
		$sqlText = "select *, ".
			"date_format(startdate_ap,'%d/%m/%Y') as start, date_format(enddate_ap,'%d/%m/%Y') as end, ".
			"date_format(storagedate_ap,'%d/%m/%Y') as stg, date_format(date_admis,'%d/%m/%Y') as admis, ".
			" replace(replace(comment_ap,'#','No'),'\n','') comment_ap_replace ".
		...
		$sqlText = "select id_role from employees e inner join plazaxemp pe on e.employee_id=pe.employee_id ".
			"inner join placexdep pd on pd.id_placexdep=pe.id_placexdep ".
			"where pe.status_plxemp='A'  and  e.employee_id=".$dtAp['0']['EMPLOYEE_ID']." ".
			"union all ".
			"select pd.id_role ".
			...
			"and pe.id_plxemp = (select max(pxe.id_plxemp) from plazaxemp pxe where pxe.employee_id = e.employee_id ) ".
			"and  e.employee_id=".$dtAp['0']['EMPLOYEE_ID'];

		$rol_emp = $dbEx->selSql($sqlText);
		
		$sqlText = "select e.employee_id, e.firstname, e.lastname ".
			"from employees e inner join apxemp ape on e.employee_id=ape.autor_ap=".$dtAp['0']['AUTOR_AP'];
		
		$sqlText2 = "select e.employee_id, e.firstname, e.lastname, pl.name_place ".
			"from employees e inner join plazaxemp pe on e.employee_id=pe.employee_id ".
			"inner join placexdep pd on pd.id_placexdep=pe.id_placexdep ".
			"inner join places pl on pl.id_place=pd.id_place inner join apxemp ape on e.employee_id=";

	
		if($rol_emp['0']['id_role'] <=3){ //Si el empleado al q se le realizo la ap es agente o supervisor
			if($rol_autor['0']['name_role']!='GERENTE DE AREA' and $dtAp['0']['ID_TPAP']!=15  and $dtAp['0']['ID_TPAP']!=16 ){
			...
			if($rol_autor['0']['name_role']!='WORKFORCE' and $dtAp['0']['ID_TPAP']!=15  and $dtAp['0']['ID_TPAP']!=16 and $dtAp['0']['TYPESANCTION_AP']!=1 and $dtAp['0']['TYPESANCTION_AP']!=2 ){
	...
	case 'ResultAutorAp':
		...
		if($dtAp['0']['ID_TPAP']>=9 and $dtAp['0']['ID_TPAP']<15){
			if($result ==1){
			    //Si han terminado las aprobaciones inactivar al empleado y la plaza activa
				...
				$sqlText = "update plazaxemp set status_plxemp='I', end_date = CURDATE() ".
					"where employee_id =".$dtAp['0']['EMPLOYEE_ID']." and status_plxemp = 'A' ";
	            $dbEx->updSql($sqlText);
	...
	case 'ResultRechazAp':
		...
		$sqlText = "select name_role from user_roles ur inner join placexdep pd on pd.id_role=ur.id_role inner join plazaxemp pe on pe.id_placexdep=pd.id_placexdep inner join employees e on e.employee_id=pe.employee_id where pe.status_plxemp='A' and e.employee_id=".$dtAutor['0']['autor_ap'];
