...
if($_POST['acc']==1){ //Ingreso de nuevo archivo
  ...
  if(isset($_FILES['flDoc']['tmp_name'])) {
    ...
    if (!copy($_FILES['flDoc']['tmp_name'], $dir.$nombre)){
	  ...
	  $sqlText = "select monitsales_attach from monitoringsales_emp where id_monitsalesemp=".$_POST['idM'];
	  $dtVer = $dbEx->selSql($sqlText);
	  if(file_exists($dir.$dtVer['0']['monitsales_attach'])){
	    unlink($dir.$dtVer['0']['monitsales_attach']);
	  }
	  //ingresamos en la base de datos
	  $sqlText = "update monitoringsales_emp set monitsales_attach='".$nombre."' where id_monitsalesemp=".$_POST['idM'];
	  $dbEx->updSql($sqlText);
